...
-   $P_{i,i+1} = p$, $P_{i,i-1} = 1 - p$, $S = \mathbb{Z}$.
...
-   Random walk: $S = \mathbb{Z}$, irreducible.
...
$$
    +   Does this limit exist? If so, will it coincide with $\pi_j$?
...
$$
    +   State $i$ is
        +   *recurrent* if $\tau_{ii} < \infty$ with probability 1 --- the chain returns to state $i$ infinitely often.
            +   **positive recurrent** if $\operatorname{E}[\tau_{ii}] < \infty$ (e.g., gambler's ruin with $p < 1/2$);
            +   null recurrent if $\operatorname{E}[\tau_{ii}] = \infty$ (e.g., gambler's ruin with $p=1/2$).
        +   *transient* if $\Pr(\tau_{ii} = \infty) > 0$. (e.g., gambler's ruin with $p > 1/2$)
...
1.  An irreducible Markov chain has the *unique* stationary distribution if and only if it is positive recurrent.
...
3.  For an irreducible, aperiodic Markov chain, if $\pi$ is such that $\pi = \pi P$, $\mathbf{1}^\top \pi = 1$, $\pi \geq 0$,
then $\lim_{n\to\infty} P_{ij}^{(n)} = \pi_j$, and the chain is positive recurrent hence ergodic.
...
-   Thus we can extend Monte Clarlo integration to Markov (non-i.i.d.) sequences --- Markov chain Monte Carlo
...
-   By sampling from this MC, expectations of functions of random variable $X \sim f$ can be reliably estimated.
...
$$
where $K(x, y) = P_{X_{n+1}\mid X_n}(y \mid x)$ is the *transition kernel* such that $\int_S K(x, y) dy = 1$.
...
1.  Sample a condidate value $\tilde{X}$ from a *proposal distribution* $g(\cdot \mid X^{(t)})$.
...
-   For each state $i$, assume that $g(i \mid i) > 0$ and there exists a neighboring state $j$ ($g(j\mid i) > 0$) such that $f(i) > f(j)$. Then
$$
...
cat("Acceptance rate = ", fit1$acceptance_rate, ", 
    Sample size = ", length(fit1$theta), "\n")
my_target2 <- log_binom_logit_posterior(x = 180, n = 200, 
                                        sigma = sigma)
system.time(
...
cat("Acceptance rate = ", fit2$acceptance_rate, ", 
    Sample size = ", length(fit2$theta), "\n")
```
...
p2 <- plot_posterior(posterior_df2, target_log_density = my_target2)
p2 + geom_vline(xintercept = theta_mle, linetype = "dashed", 
...
\begin{align*}
    ...
    \\
    X_p^{(t+1)} &\sim f_{X_p \mid X_{-p}}(\cdot \mid X_1^{(t+1)}, X_2^{(t+1)}, \dotsc, X_{p-1}^{(t+1)})
\end{align*}
$$
cf. Gauss-Seidel.
